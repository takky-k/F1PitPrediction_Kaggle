py -m pip install -r f1_pitnextlap_requirements.txtimport re
import json
import time
import argparse
import warnings
...
ADDED_TOP_NOTEBOOK_FEATURES = [
    ...
    "Compound_TyreLifeBin",
    "Compound_RaceProgressBin",
    "Driver_Race_frequency",
    "Driver_Year_frequency",
    "Stint_Compound_frequency",
    "Compound_TyreLifeBin_frequency",
    "Compound_RaceProgressBin_frequency",
]

# PitStopはpost-fact leakageの可能性があるので、特徴量から完全に落とす。
DROP_PITSTOP_FEATURES = True
...
def parse_args():
    ...
    parser.add_argument(
        "--run-name",
        type=str,
        default="no_pitstop_yearstrat_extra_features_model",
    )
...
def add_basic_features(df, params):
    ...
    # ----------------------------
    # Top-notebook inspired features
    # ----------------------------

    # RaceProgress = LapNumber / estimated total laps という前提で、総周回数を推定。
    # RaceProgressが0に近い場合も壊れないようにsafe_divideを使う。
    df["EstimatedTotalLaps"] = safe_divide(
        df["LapNumber"],
        df["RaceProgress"],
    )

    df["LapsRemaining"] = (
        df["EstimatedTotalLaps"] - df["LapNumber"]
    )

    df["TyreAgeRatio"] = safe_divide(
        df["TyreLife"],
        df["EstimatedTotalLaps"]
    )

    df["DegPerTyreLap"] = safe_divide(
        df["Cumulative_Degradation"],
        df["TyreLife"] + 1,
    )

    # Positionが小さいほど前方なので、前方争いの圧力をざっくり表す。
    df["PositionPressure"] = safe_divide(
        21 - df["Position"],
        df["LapNumber"] + 1,
    )

    # 2023 anomaly indicator
    df["is_2023"] = (
        df["Year"].astype(str) == "2023"
    ).astype("int8")

    df["Year_minus_2022"] = (
        pd.to_numeric(df["Year"], errors="coerce").fillna(2022) - 2022
    ).astype("int16")

    # Binning: tire age and race progress
    df["TyreLifeBin"] = pd.cut(
        df["TyreLife"],
        bins=[-1, 0, 3, 7, 12, 18, 25, 35, 999],
        labels=["0", "1_3", "4_7", "8_12", "13_18", "19_25", "26_35", "36plus"],
        include_lowest=True,
    ).astype(str)

    df["RaceProgressBin"] = pd.cut(
        df["RaceProgress"],
        bins=[-0.01, 0.10, 0.20, 0.35, 0.50, 0.65, 0.80, 0.90, 1.01],
        labels=["p00_10", "p10_20", "p20_35", "p35_50", "p50_65", "p65_80", "p80_90", "p90_100"],
        include_lowest=True,
    ).astype(str)
    ...
    df["Compound_TyreLifeBin"] = (
        df["Compound"].astype(str)
        + "__"
        + df["TyreLifeBin"].astype(str)
    )

    df["Compound_RaceProgressBin"] = (
        df["Compound"].astype(str)
        + "__"
        + df["RaceProgressBin"].astype(str)
    )

    df["Race_Year_Compound"] = (
        df["Race"].astype(str)
        + "__"
        + df["Year"].astype(str)
        + "__"
...
def prepare_base_features(train, test, params, logger):
    ...
    logger.log("\nAdded top-notebook inspired features:")
    for f in ADDED_TOP_NOTEBOOK_FEATURES:
        logger.log(f"- {f}")

    logger.log(f"\nDROP_PITSTOP_FEATURES: {DROP_PITSTOP_FEATURES}")
...
def get_enqueue_trial_params(scale_pos_weight):
    """
    直近で良かったtrialに近い設定を最初に試す。
    scale_pos_weightだけは現在のデータのclass imbalanceに合わせる。
    """
    return {
        "learning_rate": 0.015484014792532953,
        "num_leaves": 192,
...
def train_one_cv(
    ...
    # 上位者ノート寄せ：target × Year で層化。
    # 2023などYearごとの挙動差を各foldに均等に入れる。
    strat_key = (
        train_fe[TARGET].astype(int).astype(str)
        + "__"
...
def run_optuna(
    ...
    # 最近良かった設定を最初に試す
    study.enqueue_trial(
        get_enqueue_trial_params(scale_pos_weight)
    )
...
def main():
    ...
    logger.log(f"N_SPLITS: {N_SPLITS} (StratifiedKFold by target x Year)")
        ...
        json.dump(
            {
                ...
                "added_top_notebook_features": ADDED_TOP_NOTEBOOK_FEATURES,
                "drop_pitstop_features": DROP_PITSTOP_FEATURES,
    ...
    summary_record = {
        ...
        "n_top_notebook_features": len(ADDED_TOP_NOTEBOOK_FEATURES),
        "drop_pitstop_features": DROP_PITSTOP_FEATURES,
        "stratification": "target_x_year",
        "run_dir": str(run_dir),
    }
